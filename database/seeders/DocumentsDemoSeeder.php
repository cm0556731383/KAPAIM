...
class DocumentsDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $quote->sendTo($this->recipientsFor($deal), 'pdf', $activityLogger);

        // ----- טופס הזמנה: נשלח כטופס דיגיטלי, מולא, והתקבל -----
        $orderForm = Document::generateFor($deal, $orderFormTemplate, 'digital');
        $orderForm->sendTo($this->recipientsFor($deal), 'digital', $activityLogger);
        ...
        $contract->sendTo($this->recipientsFor($deal), 'digital', $activityLogger);
        $contract->markSigned();
        ...
        $invoice->sendTo($this->recipientsFor($deal), 'pdf', $activityLogger);
